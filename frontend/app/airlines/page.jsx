...
export default function AirlineAnalysis() {
  ...
  const [isRouteMixLoading, setIsRouteMixLoading] = useState(true);
  const [routeMixError, setRouteMixError] = useState(null);
  const [scrape, setScrape] = useState(null);
  const [isScrapeLoading, setIsScrapeLoading] = useState(true);

  useEffect(() => {
    setIsLoading(true); setError(null);
    fetch(`${API_URL}/api/airlines`)
      .then(r => { if (!r.ok) throw new Error(`Airlines request failed (${r.status})`); return r.json(); })
      .then(setData)
    ...
    fetch(`${API_URL}/api/scrape-overview`)
      .then(r => r.ok ? r.json() : null)
      .then(setScrape)
      .catch(() => setScrape(null))
      .finally(() => setIsScrapeLoading(false));
  }, []);
  ...
  useEffect(() => {
    if (!selectedAirline) { setIsRouteMixLoading(false); return; }
    // ignore guards against out-of-order responses if selectedAirline/topN change quickly.
    let ignore = false;
    setIsRouteMixLoading(true); setRouteMixError(null);
    fetch(`${API_URL}/api/airlines/${selectedAirline}/routes?top_n=${topN}`)
      .then(r => {
        if (r.status === 404) return null; // no stored route observations for this airline -- not a failure
        if (!r.ok) throw new Error(`Route mix request failed (${r.status})`);
        return r.json();
      })
      .then(routes => { if (!ignore) setRouteData(routes); })
      .catch(e => { if (!ignore) { setRouteData(null); setRouteMixError(e instanceof TypeError ? "Network error: could not reach the server. Please check your connection and try again." : e.message); } })
      .finally(() => { if (!ignore) setIsRouteMixLoading(false); });
    return () => { ignore = true; };
    ...
    <main className="page">
      <section className="grid">
        <div className="panel full">
          ...
          {error ? null : isLoading ? <p className="muted">Loading data...</p> : airlines.length ? <>
            <table>
              <thead>
        ...
        <div className="panel full">
          ...
          {isScrapeLoading ? <p className="muted">Loading data...</p> : <MultiLineChart series={scrape?.airlines || []} yLabel="Average fare (Rs)" />}
        ...
        <div className="panel full">
          ...
          {routeMixError ? <p className="notice">{routeMixError}</p> : isRouteMixLoading ? <p className="muted">Loading data...</p> : <MultiLineChart series={routeData?.routes || []} yLabel="Average fare (Rs)" />}
